...
// this document was originally written as #version 410 ...
...
FGZ Unknown:  operator==not available,reserved ...  const==not avail.  friend==not avail. 
...
#define   HALFPI	1.5707963f 
  ...
  //     J = -0.5 , 0.8660254037844386 
  //    _J = -0.5 , -0.8660254037844386
  //    _i = 0 , -1 
  //     i = 0 , 1
   
#ifndef LOG10
#define   LOG10  2.3025851f  
//#define   LOG10  2.3025850929940457 
#endif
#ifndef LOG2
#define   LOG2  0.6931472f  
...
// ToDo: these need Complex-Function counter-parts to have them work with Complex ...
...
// FGZ: this is for operations that got translated that should have, like ADD( 1.0f + 3.0f ) 
_Type1_ ADD( _Type1_ x, _Type1_ y)
{
	return x + y ;
}
...
// FGZ: this is for operations that got translated that should have, like SUB( 1.0f - 3.0f ) 
_Type1_ SUB( _Type1_ x, _Type1_ y)
{
	return x - y ;
}
...
// FGZ: this is for operations that got translated that should have, like MUL( 1.0f * 3.0f ) 
_Type1_ MUL( _Type1_ x, _Type1_ y)
{
	return x * y ;
}
...
// FGZ: this is for operations that got translated that should have, like DIV( 1.0f / 3.0f ) 
_Type1_ DIV( _Type1_ x, _Type1_ y)
{
	return x / y ;
}
...
struct deadmeat {
int XXX;
lrkgldkfgv ;  // this comes up before an error somewhere, just copy and paste elsewhere ... 
};
...
// These are Read-Only 'Input' values to the shaders. 
uniform vec2 param;
uniform int iters;
uniform int ipow_f;
uniform int OrbitFunc;  
...
int julia(in vec2 z, out vec2 fMU) {
	...
	// 	if(length(z) >= 4.0f) break;   // FGZ :  was 2.0f ...
		...
		case 3:
			...
			break;
		case 5:
			for(i=((UseEsmooth)?0:1); i<iters; i++) {
				if( x*x+y*y >= fBailout ) break;
				if( UseEsmooth ) fMU.x += Fast_eSmooth(x*x+y*y);
		...
		case 4:
			for(i=0; i<iters; i++) {  // ((UseEsmooth)?0:1)
				if( x*x+y*y >= 16.0f ) break;
				if( UseEsmooth ) fMU.x += Fast_eSmooth(x*x+y*y);
